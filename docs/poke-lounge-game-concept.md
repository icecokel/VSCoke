...
야생 조우 기본 확률은 정상적인 타일 이동 한 번당 15%다. 전국도감 1번부터 493번까지의 모든 종은 아래 세 지역 중 한 곳에서 출현한다. 포획률이 높은 종일수록 조우 가중치가 높지만 모든 종의 가중치는 1 이상이므로 각 지역 범위의 어떤 종도 제외되지 않는다. 야생 레벨은 현재 파티 평균 레벨보다 최대 5 낮은 범위부터 평균 레벨까지에서 보정된다.

| 지역      | 전국도감 범위 | 종 수 | 세대 범위 |
| --------- | ------------- | ----: | --------- |
| 서쪽 필드 | 1–151         |   151 | 1세대     |
| 광장 필드 | 152–251       |   100 | 2세대     |
| 남쪽 필드 | 252–493       |   242 | 3–4세대   |
