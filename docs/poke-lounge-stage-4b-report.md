...
casual command 응답은 room command repository가 저장한 `responseState`와 `responseRevision`을 기준으로 유지한다. 새 command commit 뒤 projection 조회 시 current room revision이 command revision과 같을 때만 competitive를 첨부한다. 그 사이 다음 commit이 완료되어 revision이 달라지면 이전 command 응답과 이벤트는 원본 snapshot을 그대로 사용한다. idempotent replay는 projection 조회나 이벤트 발행 없이 저장된 snapshot을 그대로 반환하며, 최신 상태는 fresh GET 또는 Socket subscription이 별도 consistent-current 조회로 가져온다.

PostgreSQL race 테스트는 rev7 command commit 직후 rev8 command를 commit·발행한 다음 rev7 enrichment를 진행한다. rev7 응답과 replay는 rev7 참가자 상태와 revision을 유지하고 competitive를 포함하지 않으며, rev8 이벤트는 한 번만 발행된다. 후속 GET은 rev8 참가자 상태와 competitive projection을 반환한다.
