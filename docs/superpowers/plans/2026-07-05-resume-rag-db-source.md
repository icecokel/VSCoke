...
**Goal:** Build a resume RAG chat system where database source items are the source of truth and runtime question answering reads only from vector chunks.

**Architecture:** Existing resume files are import inputs only. Import turns allowlisted raw evidence, final resume sections, strategy documents, and mapping summaries into `resume_source_items`; indexing turns vectorizable source items into `resume_vector_chunks`; chat retrieval reads vector chunks only. Natural-language answer generation uses Codex app-server; embedding provider, embedding model, and embedding dimensions remain runtime configuration decisions.

**Tech Stack:** NestJS 11, TypeORM 0.3, PostgreSQL, pgvector, Next.js 15 App Router, Auth.js/next-auth, next-intl, shadcn/ui primitives, Jest, Playwright.
...
- Vector rows denormalize citation metadata so runtime retrieval can answer without source-table joins.
...
| 4     | [Chat API](./2026-07-05-resume-rag-phase-4-chat-api.md)                           | Authenticated vector-only retrieval and grounded answer generation                         |
| 5     | [Web Chat](./2026-07-05-resume-rag-phase-5-web-chat.md)                           | Authenticated resume question UI and citation rendering                                    |

## Execution Order

1. Complete schema and import manifest design.
2. Implement allowlisted import and sanitizer.
3. Implement embedding adapter, chunker, and exact vector indexing.
4. Implement vector-only chat API and redaction.
5. Implement web chat UI after API contract stabilizes.

## Acceptance Criteria

- Source rows are inspectable in PostgreSQL as `resume_source_items`.
- Vector rows can be regenerated from DB source items.
- Vector rows contain enough metadata to cite title, source path, source key, section path, version, and caveats.
- Chat retrieval filters by embedding provider/model/dimension metadata.
- Model settings are required runtime configuration, not source defaults.
- Tests cover importer, indexer, retriever, redaction, auth behavior, and web route rendering.
