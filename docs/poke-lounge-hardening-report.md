...
구현 기준: `4e4090a` (`fix(poke-lounge):권위 대전 오류 경계 보강`)
...
일반 `POST /game/result`와 casual `POST /poke-lounge/rooms/:roomCode/result`는 언제나 client-asserted unranked다. Web은 server room 결과를 generic score endpoint로 다시 제출하지 않는다. `GET /game/ranking?gameType=POKE_LOUNGE`는 `verified-room` row만 사용자 최고 점수와 등수 계산에 사용한다.
...
PostgreSQL test data source는 `TEST_DATABASE_URL`을 필수로 요구하고 `_test` database suffix, regular DB URL과의 target 분리, `synchronize: false`를 강제한다. PR API job은 PostgreSQL 16 service와 pgvector를 준비하고 test migration show/run 후 integration/E2E를 실행한다.
...
이 감사에서는 별도 PostgreSQL `_test` database를 새로 띄우지 않아 integration/E2E를 재실행하지 않았다. 해당 근거는 PR PostgreSQL 16 workflow와 [Stage 4B 검증 기록](./poke-lounge-stage-4b-report.md)에 남아 있다.
