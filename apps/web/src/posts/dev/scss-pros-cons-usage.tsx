...
const DevScssProsConsUsagePost = () => {
  return (
    <>
      <PostParagraph>
        {
      ...
      <PostOrderedList start={1}>
        <PostListItem>
          <PostParagraph>
            <PostStrong>SCSS 개념 및 장점</PostStrong>
          </PostParagraph>
        </PostListItem>
        <PostListItem>
          <PostParagraph>
            <PostStrong>SCSS 사용방법</PostStrong>
          </PostParagraph>
        </PostListItem>
      </PostOrderedList>
      <PostHorizontalRule />
      <PostHeading3>
        <PostStrong>1. SCSS 개념 및 장점</PostStrong>
      </PostHeading3>
      <PostParagraph>
        {
      ...
      />
      <PostParagraph>
        위에서 명확히 구분이 되듯, 셀렉터 안에 셀렉터를 넣어 중첩을 시켜 가독성이 좋아졌습니다. 이게
        20라인도 안 되는 부분에서 보이는 장점인데, 1000라인이 넘어가는 코드에서는 명확하게 보이는
        장점이겠죠. HTML처럼 계층적인 구조를 SCSS로 구현하여 HTML 보듯 자연스럽게 볼 수 있습니다.
      </PostParagraph>
      <PostHorizontalRule />
      <PostHeading3>
        <PostStrong>2. SCSS 사용방법</PostStrong>
      </PostHeading3>
      <PostParagraph>
        {
      ...
      <PostParagraph>
        또한 디자인 컨셉 변경 등 컬러코드 속성을 대대적으로 변경할 때에도, 변수 선언 부만 수정하면
        되니 유지보수성도 뛰어납니다.
      </PostParagraph>
      <PostHeading3>
        <PostStrong>중첩 (</PostStrong>
        {" Nesting "}
        <PostStrong>)</PostStrong>
      </PostHeading3>
      <PostParagraph>
      ...
      <PostParagraph>
        SCSS 코드와 같이 작성을 해두면, 가독성은 물론 작성하는 개발자 또한, 효율적으로 코딩이
        가능해집니다.
      </PostParagraph>
      <PostParagraph>
        중첩기능의 장점이라고 생각되는 부분중 하나는, 복잡한 구조의 HTML에서는 각각 요소들에 맞는
        디자인을 추가하기 위해 id , class , className 등 태그마다 선언을 해줘야 하는 번거로움이
        있으나, 셀렉터 중복 기능의 특성인 하위 항목에 적용되는 부분을 잘 이해하면 HTML 태그 자체에서
        속성 값을 줄일 수 있으므로, HTML 코드의 가독성 또한 상승합니다.
      </PostParagraph>
      <PostHeading3>
        <PostStrong>임포트 (</PostStrong>
        {" Import "}
        <PostStrong>)</PostStrong>
      </PostHeading3>
      <PostParagraph>
      ...
      <PostParagraph>
        이렇게 관리하면 디폴트 디자인이 가능하고, 한쪽에서 선언해둔 컬러 코드나 폰트를 가져다가 다른
        scss파일에서 사용하기가 용이합니다. 그에 따라 유지보수 효과적으로 할 수 있다고 생각합니다.
      </PostParagraph>
      <PostParagraph>
      ...
      <PostBlockquote>
        <PostParagraph>
          ⚠️ Heads up!
          <br />
          The Sass team discourages the continued use of the @import rule. Sass will gradually phase
          it out over the next few years, and eventually remove it from the language entirely.
          Prefer the @use rule instead. (Note that only Dart Sass currently supports @use. Users of
          other implementations must use the @import rule instead.)
        </PostParagraph>
      </PostBlockquote>
      <PostParagraph>
        {
      ...
      <PostParagraph>
        <PostLink href={"https://sass-lang.com/"}>https:&#47;&#47;sass-lang.com/</PostLink>
      </PostParagraph>
      <PostParagraph>{" [Sass: Syntactically Awesome Style Sheets"}</PostParagraph>
      <PostParagraph>
        Sass is the most mature, stable, and powerful professional grade CSS extension language in
        the world.
      </PostParagraph>
      <PostParagraph>sass-lang.com](https:&#47;&#47;sass-lang.com/)</PostParagraph>
    </>
  );
};

export default DevScssProsConsUsagePost;
