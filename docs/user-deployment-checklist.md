...
- [ ] Root Directory를 `apps/web`으로 설정한다.
- [ ] Framework Preset이 `Next.js`인지 확인한다.
- [ ] Build Command가 `pnpm build`인지 확인한다.
- [ ] Install Command는 기본값을 우선 사용한다. 실패할 때만 `pnpm install --frozen-lockfile`로 override한다.
- [ ] 환경 변수를 Production에 등록한다.
  - [ ] `NEXT_PUBLIC_API_URL=https://api.icecoke.kr`
  - [ ] `AUTH_GOOGLE_ID`
  - [ ] `AUTH_GOOGLE_SECRET`
  - [ ] `AUTH_SECRET`
- [ ] Preview 환경에서도 API 호출이나 Google Auth가 필요하면 같은 env를 등록한다.
- [ ] env 변경 후 새 Vercel deployment를 생성한다. 기존 deployment에는 env 변경이 소급되지 않는다.

검증:

- [ ] PR #16의 Vercel check가 성공으로 바뀌는지 확인한다.
- [ ] Vercel 빌드 로그에서 repo root가 아니라 `apps/web` 기준으로 빌드되는지 확인한다.
- [ ] Vercel 빌드 로그에서 API 빌드가 실행되지 않는지 확인한다.
- [ ] production URL 접속을 확인한다.
...
- [ ] Vercel check가 성공하면 PR #16을 merge한다.
- [ ] Vercel 설정을 먼저 바꾸기 어렵다면, 실패 원인이 Root Directory 설정임을 확인한 뒤 의도적으로 merge할지 결정한다.
- [ ] merge 후 `main`에서 web/API 배포가 각각 기대한 경로로 실행되는지 확인한다.
