...
첫 참가자는 대기실의 방장이 된다. 2~6명의 현재 파티가 동기화되고 모든 참가자가 ready를
선택하면 방장이 1라운드를 시작한다. 이때부터 모든 참가자에게 같은 5분 준비 시간이 주어지며,
시작 뒤 신규 참가자는 받지 않는다. 정상적으로 1·2라운드를 완료하면 다음 준비는 자동
시작한다.
...
`pnpm check:poke-lounge-provenance`는 미해결 항목 때문에 의도적으로 실패한다. 기본 Vercel
