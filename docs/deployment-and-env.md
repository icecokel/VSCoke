...
```txt
apps/web -> Vercel
apps/api -> GitHub Actions -> Termux server -> PM2 -> Cloudflare Tunnel
```
...
API는 `.github/workflows/deploy-api.yml`이 배포한다.

현재 워크플로 기준:

1. `main` 브랜치에 `apps/api/**`, 루트 패키지/락파일, API 배포 워크플로 변경이 push된다.
2. GitHub Actions가 루트에서 의존성을 설치한다.
3. `pnpm --filter @vscoke/api build`로 API를 빌드한다.
4. `apps/api/dist`, `apps/api/package.json`, 루트 `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`을 tar로 묶는다.
5. Cloudflare Tunnel SSH 경로로 Termux 서버에 업로드한다.
6. `~/projects/vscoke-api`에서 production 의존성을 설치하고 PM2로 API를 재기동한다.

운영 프로세스 기준:

| 항목        | 값                          |
| ----------- | --------------------------- |
| 서버 경로   | `~/projects/vscoke-api`     |
| PM2 앱 이름 | `vscoke-api`                |
...
- 첫 production 배포가 성공하면 Termux 서버에서 `pm2 save`를 실행해 현재 프로세스 목록을 저장한다.
...
GitHub Actions secrets는 API 배포 접속 정보만 관리한다. 애플리케이션 런타임 비밀값은 Termux `.env`에서 관리한다.

현재 워크플로가 사용하는 값:

| 이름          | 설명                           |
| ------------- | ------------------------------ |
| `TERMUX_HOST` | Cloudflare Tunnel SSH hostname |
| `TERMUX_USER` | Termux SSH 사용자              |
| `TERMUX_KEY`  | Termux SSH private key         |

Cloudflare Access가 비대화형 인증을 요구하면 서비스 토큰용 secret을 추가하고 워크플로에 반영한다. 이 변경 전에는 GitHub Actions에서 SSH 접속이 실제로 성공하는지 먼저 확인한다.

서비스 토큰 방식이 필요할 때 후보 값:
...
- [ ] Vercel web 프로젝트의 Root Directory를 `apps/web`으로 변경한다.
- [ ] Vercel Production/Preview 환경에 web 환경 변수 표의 필수 값을 설정한다.
- [ ] Termux 서버의 `~/projects/vscoke-api/.env`를 현재 API 환경 변수 목록 기준으로 정리한다.
- [ ] API GitHub Actions secrets `TERMUX_HOST`, `TERMUX_USER`, `TERMUX_KEY`를 새 monorepo 저장소 기준으로 확인한다.
...
- [ ] API lint debt를 정리한 뒤 루트 `pnpm lint`에 API lint를 포함한다.
- [x] PM2 재부팅 복구 기준을 문서화한다.
- [ ] PM2 실행 명령을 `ecosystem.config.cjs`로 옮길지 검토한다.
