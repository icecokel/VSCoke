...
export function WordleBoard({ guesses, history, currentGuess, turn }: WordleBoardProps) {
  const emptyRows = Math.max(0, 5 - turn); // 6 rows total, turn 0-5. If turn=6 (game over), empty=0

  return (
    <div className="grid grid-rows-6 gap-2 w-full max-w-[350px] mx-auto mb-8">
      {/* 1. Past Guesses (Completed) */}
      {history.map((rowStatus, i) => {
        if (i >= turn) return null; // Future or current rows handled below
