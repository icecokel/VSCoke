...
**Public release risk is unresolved.** The default Vercel build does not enforce this audit, but Poke Lounge still has unresolved rights records for Pokémon names/marks, ROM-derived game data and sprite sheets, and third-party-labelled visual and map material.
...
| Pokémon sprites                           | `apps/web/public/assets/pokemon/front/`, `apps/web/public/assets/pokemon/back/`, `apps/web/public/assets/pokemon/battle/`, `apps/web/public/assets/pokemon/cataloged/`, `apps/web/public/assets/pokemon/sheets/`                                                                                                                                                                                                                                                                                                                                                                                                | Confirmed ROM-derived; owner decision required                       | The four 493-species front/back sheets are reproducibly extracted from the local HeartGold ROM by `scripts/poke-lounge/extract-rom-game-data.py`. All 30 unique legacy front/back frames match the generated sheet frames byte-for-byte; the remaining legacy paths are duplicate copies of those frames. This establishes technical origin, not distribution rights. Authorize, replace, or remove before release.                 |
...
- `scripts/poke-lounge/audio-cues.json` and `scripts/poke-lounge/render-audio-cues.py` are the current ROM-audio regeneration path. They require ignored local ROM inputs and record technical provenance only, not permission to distribute their output.
...
`docs/poke-lounge-asset-provenance.json` records all 70 audited public files with local SHA-256 values. The audio manifest and eight ROM-derived audio rows are `blocked` until a release owner records a rights decision; the other 61 rows also remain `blocked`. `pnpm check:poke-lounge-provenance` validates the public-file coverage, hashes, source record, approval fields, and attribution fields. The command remains intentionally strict, but the default Vercel build only enforces it when `POKE_LOUNGE_PROVENANCE_STRICT=1` is set.
