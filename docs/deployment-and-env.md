...
2. Termux 서버의 self-hosted runner가 job을 실행한다.
3. runner 작업 디렉터리에서 의존성을 설치한다.
4. `pnpm --filter @vscoke/api build`로 API를 빌드한다.
5. `~/projects/vscoke-api/.env`가 있는지 확인한다.
6. `~/projects/vscoke-api/.next-release`에 `apps/api/dist`, `apps/api/package.json`, 루트 `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`을 staging한다.
7. staging 경로에서 production 의존성을 설치한다.
8. staging이 성공하면 `~/projects/vscoke-api`로 release 파일을 복사하고 PM2로 API를 재기동한다.
9. Termux 내부 `http://127.0.0.1:$PORT/api-json`와 공개 `API_HEALTH_URL`을 smoke test한다.
...
| runtime     | Node.js 20 이상, Corepack, PM2        |
...
| `API_DEPLOY_DIR` | Termux 배포 디렉터리. GitHub Actions repository variable이다. 기본값은 `~/projects/vscoke-api`              |
...
- [ ] Termux 서버에 self-hosted runner를 설치하고 `termux`, `vscoke-api` label을 부여한다.
