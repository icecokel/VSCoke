...
야생 조우 기본 확률은 정상적인 타일 이동 한 번당 15%다. 전국도감 1번부터 493번까지의 모든 종은 아래 세 지역 중 한 곳에서 출현한다. 포획률이 높은 종일수록 조우 가중치가 높지만 모든 종의 가중치는 1 이상이므로 각 지역 범위의 어떤 종도 제외되지 않는다. 야생 레벨은 현재 파티 평균 레벨을 중심으로 약 ±5 범위에서 보정된다.
